	...
	for _, item := range resp.Pairs {
		...
		case *v1.CheckBulkPermissionsPair_Item:
			...
			if cobrautil.MustGetBool(cmd, "explain") || cobrautil.MustGetBool(cmd, "schema") {
				err = displayDebugInformationIfRequestedWithOptions(cmd, responseType.Item.DebugTrace, nil, false, true)
				if err != nil {
					return err
				}
			}
...
func displayDebugInformationIfRequested(cmd *cobra.Command, debug *v1.DebugInformation, trailerMD metadata.MD, hasError bool) error {
	return displayDebugInformationIfRequestedWithOptions(cmd, debug, trailerMD, hasError, false)
}

func displayDebugInformationIfRequestedWithOptions(cmd *cobra.Command, debug *v1.DebugInformation, trailerMD metadata.MD, hasError bool, skipHTML bool) error {
	if cobrautil.MustGetBool(cmd, "explain") || cobrautil.MustGetBool(cmd, "schema") || (cobrautil.MustGetBool(cmd, "html") && !skipHTML) {
		...
		if cobrautil.MustGetBool(cmd, "html") && !skipHTML {
			htmlOutput := printers.DisplayCheckTraceHTML(debugInfo.Check, hasError)
